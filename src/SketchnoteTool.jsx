// SketchnoteTool.jsx — Main component: Wizard + 8 SVG renderers + Color control
...
import { JourneySVG, PosterSVG, FlowSVG } from './Layouts.jsx';
  ...
  const generate = React.useCallback(async () => {
    ...
    try {
      const res = await fetch(IMAGE_WORKER_URL, {
        ...
        body: JSON.stringify({ title: data.title, subtitle: data.subtitle, mood: data.mood, sections: (data.sections || []).map(s => ({ title: s.title, scene: s.scene, items: s.items })), footer: data.footer, cm: data.cm }),
      });
      const ct = res.headers.get('content-type') || '';
      if (ct.includes('application/json')) { const errJson = await res.json(); throw new Error(errJson.detail || errJson.error || 'Fehler'); }
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const blob = await res.blob();
      const reader = new FileReader();
      reader.onloadend = () => { if (reader.result) setImgSrc(reader.result); else setErr('Bild konnte nicht geladen werden'); };
      reader.readAsDataURL(blob);
    } catch (e) { setErr(e.message || 'Bildgenerierung fehlgeschlagen'); } finally { setLoading(false); }
  }, [data]);
  React.useEffect(() => {
    const key = JSON.stringify(data.title + (data.sections || []).map(s => s.title).join());
    if (key !== dataKey.current) { dataKey.current = key; generate(); }
  }, [data, generate]);
  if (loading) return (<div style={{ width: '100%', aspectRatio: '1536/1024', background: '#fff', borderRadius: 12, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', border: '2px solid #f0e0d8' }}><div style={{ fontSize: 48, animation: 'spin 1.5s linear infinite' }}>🎨</div><p style={{ fontFamily: 'Caveat,cursive', fontSize: 22, color: '#E8584F', marginTop: 16 }}>KI zeichnet...</p><style>{`@keyframes spin{to{transform:rotate(360deg);}}`}</style></div>);
  if (err) return (<div style={{ width: '100%', aspectRatio: '1536/1024', background: '#fff', borderRadius: 12, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', border: '2px solid #f0e0d8' }}><p style={{ fontFamily: 'Patrick Hand,cursive', fontSize: 16, color: '#E8584F', maxWidth: 400, textAlign: 'center' }}>⚠ {err}</p><button onClick={generate} style={{ marginTop: 12, padding: '8px 24px', borderRadius: 10, border: 'none', background: '#E8584F', color: '#fff', fontFamily: 'Caveat,cursive', fontSize: 16, cursor: 'pointer' }}>🔄 Erneut</button></div>);
  if (imgSrc) return (<div style={{ position: 'relative' }}><img id="sketchnote-ai-img" src={imgSrc} alt={data.title} style={{ display: 'block', width: '100%', borderRadius: 12, border: '2px solid #f0e0d8' }} /><button onClick={generate} style={{ position: 'absolute', top: 12, right: 12, padding: '6px 14px', borderRadius: 8, border: 'none', background: 'rgba(255,255,255,0.92)', color: '#E8584F', fontFamily: 'Caveat,cursive', fontSize: 14, cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.15)' }}>🎲 Neu</button></div>);
  return null;
}
...
/* ═══════════════════════════════════════════
   ...
   ═══════════════════════════════════════════ */
const SVG_SYSTEM = `Du bist ein Sketchnote-Designer. Generiere ein SVG im Bikablo-Stil. viewBox="0 0 1100 780". Schriften: Caveat (Titel,700), Patrick Hand (Text). Importiere: @import url('https://fonts.googleapis.com/css2?family=Caveat:wght@400;600;700&family=Patrick+Hand&display=swap'). Akzent: #E8584F. Text: #2D2D2D. Hintergrund: weiß. Figuren: Runde Köpfe, Punkte für Augen, Kurve für Mund, Haare als path, Körper als Linien. NUR SVG-Code, kein Markdown. Beginne mit <svg, ende mit </svg>. Alle Texte Deutsch. Mind. 4 Figuren.`;

function SketchLayout({ data }) {
  ...
  const generate = React.useCallback(async () => {
    ...
    try {
      const secText = (data.sections || []).map((s, i) => `${i + 1}. "${s.title}" — ${(s.items || []).join(', ')}`).join('\n');
      const userPrompt = `Sketchnote: "${data.title}"${data.subtitle ? ` (${data.subtitle})` : ''}\nSektionen:\n${secText}\n${data.cm ? `Botschaft: ${data.cm}` : ''}\nStimmung: ${data.mood || 'optimistisch'}`;
      const apiUrl = import.meta.env.VITE_API_URL || '/api/generate';
      const res = await fetch(apiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: 'claude-sonnet-4-20250514', max_tokens: 12000, system: SVG_SYSTEM, messages: [{ role: 'user', content: userPrompt }] }) });
      ...
      setSvgCode(match[0]);
    } catch (e) { setErr(e.message || 'SVG-Generierung fehlgeschlagen'); } finally { setLoading(false); }
  }, [data]);
  React.useEffect(() => {
    const key = JSON.stringify(data.title + (data.sections || []).map(s => s.title).join());
    if (key !== dataKey.current) { dataKey.current = key; generate(); }
  }, [data, generate]);
  if (loading) return (<div style={{ width: '100%', aspectRatio: '1100/780', background: '#fff', borderRadius: 12, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', border: '2px solid #f0e0d8' }}><div style={{ fontSize: 48, animation: 'spin 1.5s linear infinite' }}>✏️</div><p style={{ fontFamily: 'Caveat,cursive', fontSize: 22, color: '#E8584F', marginTop: 16 }}>Claude zeichnet...</p><style>{`@keyframes spin{to{transform:rotate(360deg);}}`}</style></div>);
  if (err) return (<div style={{ width: '100%', aspectRatio: '1100/780', background: '#fff', borderRadius: 12, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', border: '2px solid #f0e0d8' }}><p style={{ fontFamily: 'Patrick Hand,cursive', fontSize: 16, color: '#E8584F', maxWidth: 400, textAlign: 'center' }}>⚠ {err}</p><button onClick={generate} style={{ marginTop: 12, padding: '8px 24px', borderRadius: 10, border: 'none', background: '#E8584F', color: '#fff', fontFamily: 'Caveat,cursive', fontSize: 16, cursor: 'pointer' }}>🔄 Erneut</button></div>);
  if (svgCode) return (<div style={{ position: 'relative' }}><div id="sketchnote-svg-container" dangerouslySetInnerHTML={{ __html: svgCode.replace('<svg', '<svg id="sketchnote-svg" style="width:100%;height:100%;border-radius:12px"') }} /><button onClick={generate} style={{ position: 'absolute', top: 12, right: 12, padding: '6px 14px', borderRadius: 8, border: 'none', background: 'rgba(255,255,255,0.92)', color: '#E8584F', fontFamily: 'Caveat,cursive', fontSize: 14, cursor: 'pointer', boxShadow: '0 2px 8px rgba(0,0,0,0.15)' }}>🎲 Neu</button></div>);
  return null;
}

const _sl = (t) => (t || 'sketchnote').replace(/[^a-zA-Z0-9äöüß]/g, '-').replace(/-+/g, '-').slice(0, 40);
...
export default function SketchnoteTool({ lang: propLang, sharedPal, sharedBaseColor, sharedMoodKey, onHome }) {
  ...
  if (ph === 'result' && sn) {
    ...
    try {
      ...
      else if (rs === 'journey') svg = <JourneySVG data={sn} pal={pal} />;
      else if (rs === 'poster') svg = <PosterSVG data={sn} pal={pal} />;
      else if (rs === 'flow') svg = <FlowSVG data={sn} pal={pal} />;
      ...
      <div style={{ minHeight: '100vh', background: 'linear-gradient(145deg,#FEFCFB,#F5F0EB)' }}>
        ...
        <div style={{ padding: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 4, marginBottom: 10, justifyContent: 'center' }}>
            ...
            <button onClick={() => {
              if (rs === 'aigen') { const im = document.getElementById('sketchnote-ai-img'); if (im?.src) { const a = Object.assign(document.createElement('a'), { href: im.src, download: `sn-${_sl(sn.title)}.png` }); document.body.appendChild(a); a.click(); document.body.removeChild(a); } }
