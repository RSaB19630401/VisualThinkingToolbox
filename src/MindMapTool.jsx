// MindMapTool.jsx — Mind Map generator + editor with import/export, delete, undo
...
function MindMapSVG({ data, pal }) {
  ...
  return (<svg id="sketchnote-svg" viewBox={`0 0 ${W} ${H}`} xmlns="http://www.w3.org/2000/svg" style={{ width: '100%', height: '100%', background: pal.bg, borderRadius: 12 }}>
    ...
    {branches.map((br, i) => {
      ...
      return (<g key={i}>
        <path d={`M${cx},${cy} Q${(cx + bx2) / 2 + (brRng() - 0.5) * 30},${(cy + by2) / 2 + (brRng() - 0.5) * 30} ${bx2},${by2}`} fill="none" stroke={col} strokeWidth="3" opacity="0.5" />
        ...
        {children.map((ch, j) => {
          ...
          return (<g key={j}>
            <path d={`M${bx2},${by2} Q${(bx2 + chx) / 2 + (brRng() - 0.5) * 12},${(by2 + chy) / 2 + (brRng() - 0.5) * 12} ${chx},${chy}`} fill="none" stroke={col} strokeWidth="1.8" opacity="0.35" />
...
export default function MindMapTool({ lang, pal, baseColor, onBack }) {
  ...
  const withUndo = (fn) => setData(prev => { if (!prev) return prev; undoRef.current = JSON.parse(JSON.stringify(prev)); return fn(JSON.parse(JSON.stringify(prev))); });
  ...
  const updBranch = (i, f, v) => withUndo(d => { d.branches[i][f] = v; return d; });
  const updChild = (bi, ci, v) => withUndo(d => { d.branches[bi].children[ci] = v; return d; });
  const addChild = (bi) => withUndo(d => { d.branches[bi].children.push('...'); return d; });
  const delChild = (bi, ci) => withUndo(d => { d.branches[bi].children.splice(ci, 1); return d; });
  const addBranch = () => withUndo(d => { d.branches.push({ label: 'Neuer Ast', color: ['primary', 'secondary', 'accent'][d.branches.length % 3], children: ['...'] }); return d; });
  const delBranch = (i) => withUndo(d => { d.branches.splice(i, 1); return d; });
  const updTitle = (v) => withUndo(d => { d.title = v; return d; });

  // JSON Export
  const exportJSON = () => {
    if (!data) return;
    const sl = (data.title || 'mindmap').toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 30);
    dlB(new Blob([JSON.stringify({ v: 1, tool: 'mindmap', topic, data, at: new Date().toISOString() }, null, 2)], { type: 'application/json' }), `mm-${sl}.json`);
  };
  ...
  const importJSON = (e) => {
    ...
    reader.onload = ev => {
      try {
        ...
        if (j.data?.title && j.data?.branches) { setData(j.data); setTopic(j.topic || j.data.title); }
        else if (j.title && j.branches) { setData(j); setTopic(j.title); }
  ...
  if (fs) return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: '#fff', zIndex: 9999, overflow: 'auto', WebkitOverflowScrolling: 'touch' }}>
      ...
      <div style={{ width: '100%', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 10 }}>
        <div style={{ width: '100%', maxWidth: 1200, touchAction: 'pinch-zoom' }}><MindMapSVG data={data} pal={pal} /></div>
      </div>
    </div>
  );
          ...
          {data.branches.map((br, bi) => (
            <div key={bi} style={{ marginBottom: 10, padding: 10, borderRadius: 8, border: `1px solid ${gc(pal, br.color || 'primary')}40`, background: '#fafafa' }}>
              <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginBottom: 6 }}>
                ...
                <button onClick={() => delBranch(bi)} style={{ background: 'none', border: 'none', color: '#E8584F', cursor: 'pointer', fontSize: 18, padding: '0 4px' }} title="Ast löschen">✕</button>
              </div>
              {br.children.map((ch, ci) => (
                <div key={ci} style={{ display: 'flex', gap: 6, marginBottom: 3, paddingLeft: 20, alignItems: 'center' }}>
                  ...
                  <button onClick={() => delChild(bi, ci)} style={{ background: 'none', border: 'none', color: '#E8584F', cursor: 'pointer', fontSize: 16, padding: '0 4px' }} title="Löschen">✕</button>
                </div>
              ))}
